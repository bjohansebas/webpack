...
class Compiler {
	...
	_cleanupLastCompilation() {
		if (this._lastCompilation !== undefined) {
			// experiments.incremental: keep the module graph and modules alive so the
			// next compilation can reuse them; only the chunk graph is rebuilt (seal).
			const incremental = this.options.experiments.incremental;
			// child compilations are never persisted — always fully cleaned up
			...
			if (!incremental) this._lastCompilation = undefined;
		}
	}
	...
	compile(callback) {
		...
		this.hooks.beforeCompile.callAsync(params, (err) => {
					...
					process.nextTick(() => {
						...
						compilation.finish((err) => {
							...
							compilation.seal((err) => {
								...
								// experiments.incremental: release the parsers of reused
								// modules. A parser's hooks are tapped (e.g. DefinePlugin)
								// with closures capturing the compilation, so keeping them on
								// durable modules would keep every previous compilation alive.
								if (
									this.options.experiments.incremental &&
									compilation._incrementalBaseModules !== undefined
								) {
									for (const module of compilation._incrementalBaseModules) {
										module.cleanupParserForCache();
									}
								}
			...
			if (compilation._incrementalRebuild) {
				// run make plugins for their per-compilation seeding (prefetch chains,
				// dynamic entries); entry re-adds are deduped and unchanged reached
				// modules short-circuit, so this stays O(seed), not an O(N) re-walk.
				// Then detect and rebuild only the modules whose inputs changed.
				compilation._incrementalNormalModuleFactory =
					params.normalModuleFactory;
				// snapshot the persisted module count before make seeds/rebuilds so
				// _incrementalMake can tell if make itself changed the graph structure.
				// Exclude runtime modules — they are seal artifacts (build-time execution
				// leaves some behind, and a full seal re-derives them), not source modules
				// whose add/remove should force a full seal.
				let preMakeCount = 0;
				for (const m of compilation.modules) {
					if (m.type !== "runtime") preMakeCount++;
				}
				compilation._incrementalPreMakeModuleCount = preMakeCount;
				compilation._incrementalMakePhase = true;
				this.hooks.make.callAsync(compilation, (err) => {
					if (err) return afterMake(err);
					compilation._incrementalMake(params.normalModuleFactory, afterMake);
				});
			} else {
				this.hooks.make.callAsync(compilation, afterMake);
			}
		});
	}
