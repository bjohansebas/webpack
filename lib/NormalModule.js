...
class NormalModule extends Module {
	...
	/**
	 * Releases the cached parser only. Used between incremental rebuilds: a parser
	 * is only needed while building and its hooks are tapped by plugins (e.g.
	 * DefinePlugin) with closures that capture the compilation, so keeping it on a
	 * reused module would keep the whole previous compilation alive. The generator
	 * is kept (unchanged modules are re-code-generated each seal without a rebuild).
	 * @returns {void}
	 */
	...
	/**
	 * Re-acquire parser and generator from the current factory so an incremental
	 * rebuild picks up this compilation's plugin hooks (e.g. DefinePlugin values),
	 * instead of reusing the ones captured at first factorization.
