...
class NormalModule extends Module {
	 ...
	 * @param {EXPECTED_ANY} normalModuleFactory the normal module factory
	 * @returns {void}
	 */
	restoreParserAndGenerator(normalModuleFactory) {
	...
	build(options, compilation, resolver, fs, callback) {
		...
		return this._doBuild(options, compilation, resolver, fs, hooks, (err) => {
			...
			const handleBuildDone = () => {
				...
				// incremental: fold in this module's factory resolve dependencies so the
				// snapshot invalidates when a loader/package.json resolution changes
				const resolveDeps =
					compilation._incrementalModuleResolveDeps &&
					compilation._incrementalModuleResolveDeps.get(this);
				if (resolveDeps !== undefined && resolveDeps) {
					if (resolveDeps.fileDependencies) {
						fileDependencies.addAll(resolveDeps.fileDependencies);
					}
					if (resolveDeps.contextDependencies) {
						contextDependencies.addAll(resolveDeps.contextDependencies);
					}
					if (resolveDeps.missingDependencies) {
						missingDependencies.addAll(resolveDeps.missingDependencies);
					}
				}
				if (nonAbsoluteDependencies !== undefined) {
