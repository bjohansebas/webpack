	...
	executeModule(module, options, callback) {
		...
		processAsyncTree(
			...
			(module, push, callback) => {
				...
				// incremental: a referenced module was not necessarily queued this
				// compilation, so drive its build + dependency processing directly to
				// make the aggregated subtree complete and fresh (build-time targets
				// carry no pitch loaders, so this never re-enters importModule).
				// KNOWN LIMITATION: re-processing an unchanged subtree module re-adds its
				// outgoing connections (setResolvedModule never dedupes), which can
				// accumulate across rebuilds. Clearing them first (removeReasons/
				// removeStaleOutgoingConnections) breaks execution — the connections feed
				// the build-time ChunkGraph — so a proper fix needs build-time execution
				// to own a scratch graph rather than mutate the durable one.
				if (this._incrementalRebuild) {
					this.buildModule(module, (err) => {
						if (err) return callback(err);
						this.processModuleDependencies(module, (err) => {
