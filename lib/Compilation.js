...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		// incremental: modules rebuilt this compilation; build-time execution treats
		// everything else as persisted-ready
		/** @type {Set<Module> | undefined} */
		this._incrementalChangedModules = undefined;
		...
		/** @type {WeakMap<Module, { fileDependencies?: LazySet<string> | Set<string>, contextDependencies?: LazySet<string> | Set<string>, missingDependencies?: LazySet<string> | Set<string> }>} */
	...
	_buildModule(module, callback) {
		...
		if (
			...
			typeof (
				/** @type {EXPECTED_ANY} */ (module).restoreParserAndGenerator
			) === "function"
		) {
			/** @type {EXPECTED_ANY} */ (module).restoreParserAndGenerator(
	...
	_addOriginResolveDeps(originModule, fr) {
		// only MISSING probes matter for re-resolution: a path that was absent at
		// resolve time later appearing (an inner node_modules, a higher-priority
		// extension) changes where the dependency resolves. The resolved file itself
		// (fileDependencies) and probed dirs (contextDependencies) are excluded — a
		// change to the dependency's own content must NOT rebuild its importer.
		if (!fr.missingDependencies) return;
		let missing = this._incrementalOriginResolveDeps.get(originModule);
		if (missing === undefined) {
			missing = new LazySet();
			this._incrementalOriginResolveDeps.set(originModule, missing);
		}
		missing.addAll(fr.missingDependencies);
	}
			...
			([module, missing], cb) => {
				this.fileSystemInfo.createSnapshot(
					...
					(err, snapshot) => {
						...
						/** @type {EXPECTED_ANY} */
						(module)._incrementalResolveSnapshot = snapshot;
						cb();
					}
				);
			},
			(err) => callback(/** @type {WebpackError | null | undefined} */ (err))
		);
	}
		...
		this.factorizeModule(
			...
			(err, factoryResult) => {
				...
				if (err) {
					...
					if (this.options.experiments.incremental && originModule) {
				...
				this.addModule(newModule, (err, _module) => {
					...
					// incremental: fold the factory's resolve dependencies (loader/
					// package.json probes) into this module so its build snapshot covers
					// them — else needBuild misses a changed loader resolution (the deps
					// otherwise land only on compilation.fileDependencies, module-blind)
					if (this.options.experiments.incremental) {
						const fr = /** @type {ModuleFactoryResult} */ (factoryResult);
						if (
							fr.fileDependencies ||
							fr.contextDependencies ||
							fr.missingDependencies
						) {
							// the created module's own resolution (its loaders/resource) —
							// folded into its build snapshot (needBuild covers it)
							this._incrementalModuleResolveDeps.set(module, {
								...
								missingDependencies: fr.missingDependencies
							});
							// the ORIGIN module's resolution of this dependency: where it
							// resolved can change (an inner node_modules appears, a managed
							// package version bumps) with no edit to the origin's own inputs,
							// so its build snapshot (created before this dep factorized)
							// can't see it — accumulate for a separate resolve snapshot.
							if (originModule !== null && originModule !== undefined) {
				...
				const onJobDone = (err) => {
					...
					if (--inProgress === 0) {
						...
						if (this._incrementalRebuild) {
							this.moduleGraph.flushUnassignedConnections(module);
						}
						callback(null, module);
					}
				};
		...
		asyncLib.each(
			...
			(module, cb) => {
				module.needBuild(context, (err, need) => {
					...
					const resolveSnapshot = /** @type {EXPECTED_ANY} */ (module)
						._incrementalResolveSnapshot;
			...
			(err) => {
				...
				this._incrementalChangedModules = new Set(changed);
				...
				for (const m of changed) {
					this.moduleGraph.getExportsInfo(m).resetProvidedExports();
				}
				if (changed.length === 0) {
	...
	_refactorizeModule(module, normalModuleFactory, callback) {
		...
		this.handleModuleCreation(
			...
			(err, newModule) => {
				...
				this.moduleGraph.removeStaleOutgoingConnections(module, staleDeps);
				if (built !== module) this._incrementalRefactorReplaced = true;
	...
	executeModule(module, options, callback) {
		...
		processAsyncTree(
			...
			(module, push, callback) => {
				...
				// incremental: the durable graph is only partially re-processed this
				// compilation, so a referenced module may be un-queued (or unresolved) or carry
				// stale connections. Drive its build + dependency processing directly (the
				// queues dedupe with any concurrent rebuild) so the aggregated subtree is
				// complete and fresh — build-time targets carry no pitch loaders, so this
				// never re-enters importModule. A full make already queued everything.
				if (this._incrementalRebuild) {
					this.buildModule(module, (err) => {
						if (err) return callback(err);
						this.processModuleDependencies(module, (err) => {
